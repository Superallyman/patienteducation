...
const DOCTORS: Doctor[] = [
  { id: 'doc1', name: 'Dr. Joe Hoja' },
  { id: 'doc2', name: 'Dr. Alexander Jobe' },
  { id: 'doc3', name: 'Dr. Bella Sudit' },
  { id: 'doc4', name: 'Dr. Trudy Gibson' },
  { id: 'doc5', name: 'Dr. Junqon Jung' },
];
...
const MedicalReportGenerator: React.FC<MedicalReportGeneratorProps> = () => {
          ...
          <html>
            <head>
              ...
              <style>
                ...
                .report-header h1 { margin: 0 0 5px 0; font-size: 26px; color: #2c5282; } /* Darker blue */
                .report-header p { margin: 4px 0; font-size: 13px; color: #555; }
                .patient-info p, .provider-info p { margin: 4px 0; font-size: 14px; }
                ...
                .topic-title-print { display: block; margin-top: 12px; margin-bottom: 5px; font-size: 15px; font-weight: bold; color: #4A5568; } /* Tailwind gray-700 */
                .topic-notes-print { margin-left: 5px; font-size: 14px; white-space: pre-wrap; }
                .list-item-print { margin-left: 25px; list-style-type: disc; } /* For list items */
  ...
  const RenderPatientReport: React.FC<{ data: ReportData }> = ({ data }) => {
    const processContentForDisplay = (content: string) => {
      return content.split('\n').map((line, index) => {
        if (line.trim() === '---') {
          return <hr key={index} className="my-6 border-gray-300 dark:border-gray-600 print-separator" />;
        }
        if (line.startsWith('**') && line.endsWith('**')) {
          return <strong key={index} className="block mt-4 mb-2 text-md font-semibold text-gray-800 dark:text-gray-200 topic-title-print">{line.substring(2, line.length - 2)}</strong>;
        }
        if (line.match(/^(\s*)(-|\*|\d+\.)\s+/)) { // Matches list items with optional leading spaces
            const Match = line.match(/^(\s*)(-|\*|\d+\.)\s+(.*)/);
            if (Match) {
                 return <li key={index} className="ml-6 list-disc text-gray-700 dark:text-gray-300 topic-notes-print list-item-print">{Match[3]}</li>;
            }
        }
        // For regular lines of text within notes
        if (line.trim() !== '') {
             return <p key={index} className="text-gray-700 dark:text-gray-300 topic-notes-print">{line}</p>;
        }
    ...
    return (
      <div ref={reportPreviewRef} className="mt-8 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 mb-3 text-sm patient-info">
          ...
          <p><strong>Date of Visit:</strong> {new Date(data.dateOfVisit + 'T00:00:00Z').toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' })}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 mb-5 text-sm provider-info">
          <p><strong>Doctor:</strong> {data.doctorName}</p>
          {data.scribeName && <p><strong>Scribe:</strong> {data.scribeName}</p>}
        </div>
        ...
        <div className="report-content space-y-3">
          {processContentForDisplay(data.reportContent)}
        </div>
        
        <div className="report-footer mt-10 pt-6 border-t border-gray-300 dark:border-gray-600 text-center">
          <p className="text-xs text-gray-500 dark:text-gray-400">
            If you have any questions regarding this summary,<h3> <b>please contact {data.scribeName} at their email: {data.scribeEmail}</b></h3>.
          </p>
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Thank you for choosing {data.clinicName || "our clinic"}. We appreciate your trust in our care.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto bg-gray-50 dark:bg-gray-900 rounded-xl shadow-2xl space-y-6 font-sans transition-colors duration-300">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              type="text" id="patientName" value={patientName} onChange={handlePatientNameChange}
          ...
          <div>
            <label htmlFor="dateOfVisit" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              Date of Visit:
            </label>
            <input
              type="date" id="dateOfVisit" value={dateOfVisit} onChange={handleDateChange}
              className="mt-1 block w-full pl-3 pr-3 py-2 text-base border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
              required
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select id="scribe" value={selectedScribe} onChange={handleScribeChange}
              className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
            >
              <option value="">--No Scribe--</option>
              {SCRIBES.map((scribe) => (<option key={scribe.id} value={scribe.id}>{scribe.name}</option>))}
            </select>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3 p-3 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700/30 max-h-60 overflow-y-auto">
            {Object.entries(TOPICS).map(([id, topic]) => (
              <label key={id} className="flex items-center space-x-2 text-sm text-gray-800 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 rounded-md transition-colors">
                <input
                  type="checkbox"
        ...
        {/* Report Content Area */}
        <div>
          <label htmlFor="report-code" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
            Visit Summary Details (Auto-generated from selected topics, edit as needed):
          </label>
          <textarea
            id="report-code" value={code} onChange={handleCodeChange} rows={18}
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            The content above will form the main body of the patient report. You can edit it directly.
            Use `**Bold Text**` for subheadings and `- List item` or `1. List item` for lists.
            Use `---` on a new line to create a separator between topic sections.
          </p>
        </div>
